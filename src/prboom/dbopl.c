...
static Bit16s *WaveTable = NULL; // Allocated in PSRAM (8KB)
...
#if ( DBOPL_WAVE == WAVE_TABLEMUL )
static Bit16u *MulTable = NULL; // Allocated in PSRAM (768B)
#endif
...
void DBOPL_InitTables( void ) {
  ...
  if ( doneTables )
    return;
  doneTables = TRUE;
  // Allocate lookup tables in PSRAM to save ~9KB DRAM
  if (!WaveTable)
    WaveTable = (Bit16s *)heap_caps_calloc(8 * 512, sizeof(Bit16s), MALLOC_CAP_SPIRAM);
#if ( DBOPL_WAVE == WAVE_TABLEMUL )
  if (!MulTable)
    MulTable = (Bit16u *)heap_caps_calloc(384, sizeof(Bit16u), MALLOC_CAP_SPIRAM);
#endif
#if ( DBOPL_WAVE == WAVE_HANDLER ) || ( DBOPL_WAVE == WAVE_TABLELOG )
