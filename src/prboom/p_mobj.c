...
boolean P_SetMobjState(mobj_t* mobj,statenum_t state)
  ...
  static statenum_t *seenstate_tab = NULL; // PSRAM-allocated transition table
  if (!seenstate_tab) {
    seenstate_tab = (statenum_t *)heap_caps_calloc(NUMSTATES, sizeof(statenum_t), MALLOC_CAP_SPIRAM);
  }
  statenum_t *seenstate = seenstate_tab;      // pointer to table
  static int recursion;                       // detects recursion
  statenum_t i = state;                       // initial state
  boolean ret = true;                         // return value
  statenum_t tempstate[64];                   // reduced for stack safety
