...
static Chip *opl_chip = NULL; // Allocated in PSRAM (~4KB)
...
int OPL_Init (unsigned int rate)
{
    ...
    mix_buffer = (int32_t *)heap_caps_malloc(opl_sample_rate * sizeof(int32_t), MALLOC_CAP_SPIRAM);

    // Create the emulator structure:

    // Create the emulator structure in PSRAM:
    DBOPL_InitTables();
    if (!opl_chip)
        opl_chip = (Chip *)heap_caps_calloc(1, sizeof(Chip), MALLOC_CAP_SPIRAM);
    Chip__Chip(opl_chip);
