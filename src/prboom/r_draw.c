...
static byte *byte_tempbuf;  /* PSRAM-allocated */
#ifndef NOTRUECOLOR
...
byte *translationtables;  /* PSRAM-allocated */
...
#define R_FLUSHHEADTAIL_FUNCNAME R_FlushHT8
#define R_FLUSHQUAD_FUNCNAME R_FlushQuad8
#include "r_drawcolpipeline_inl.h"
...
#define MAXTRANS 3
  ...
  // Allocate rendering buffers from PSRAM (saves ~1.7KB DRAM)
  if (!byte_tempbuf)
    byte_tempbuf = (byte *)heap_caps_calloc(MAX_SCREENHEIGHT * 4, 1, MALLOC_CAP_SPIRAM);
  if (!translationtables)
    translationtables = (byte *)heap_caps_calloc(3 * 256, 1, MALLOC_CAP_SPIRAM);
