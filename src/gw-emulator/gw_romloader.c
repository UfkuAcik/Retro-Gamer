...
{
   GW_ROM = (unsigned char*)malloc(GW_ROM_SIZE_MAX);

   /* src pointer to the ROM data in the external flash (raw or LZ4) */
   const unsigned char *src = (unsigned char *)GW_ROM_DATA;
   ...
   unsigned int rom_size_src  = GW_ROM_DATA_LENGTH;
   unsigned int rom_size_dest = GW_ROM_DATA_LENGTH;

   /* 1st part on FLASH before JPEG */
   unsigned int rom_size_compressed_src  = GW_ROM_DATA_LENGTH;
   ...
   if (memcmp(src, ROM_CPU_SM510, 3) == 0)
   {
      ...
      memcpy(dest, src, GW_ROM_DATA_LENGTH);
      printf("ROM2RAM done\n");

      rom_size_src = GW_ROM_DATA_LENGTH;
   ...
   else if((rom_size_compressed_src+8) != GW_ROM_DATA_LENGTH)
   {
      ...
      JpegSrc = (uint32_t)&GW_ROM_DATA[rom_size_compressed_src+8];
